...
// Use an enumeration (inner class) to represent the seeds and cell contents
	public enum Seed {
		EMPTY, CROSS, NOUGHT, starter
	}

	public Seed currentPlayer; // the current player
...
// Setup the status bar (JLabel) to display status message
	...
	public void initGame() {
		...
		do {
			String answer = JOptionPane.showInputDialog("Who would like to go first? X or O ?");
			if (answer.equals("X") || answer.equals("x")) {
				JOptionPane.showMessageDialog(null, "X goes First");
				starter = Seed.CROSS;
				x = 1;
			} else if (answer.equals("O") || answer.equals("o")) {
				JOptionPane.showMessageDialog(null, "O goes First");
				starter = Seed.NOUGHT;
				x = 1;
			}
		} while (x == 0);

		JOptionPane.showMessageDialog(null, "What color would you like X to be?");
		Rainbow = JColorChooser.showDialog(null, "Choose a color", null);
		JOptionPane.showMessageDialog(null, "What color would you like O to be?");
		RainBow = JColorChooser.showDialog(null, "Choose a color", null);
		...
		currentPlayer = starter; // cross plays first
	}
...
// Run GUI codes in the Event-Dispatching thread for thread safety
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				new TicTacToe3D(); // Let the constructor do the job
			}
		});
	}
}